      ...
      <Container>
        ...
        <div className="mt-14 grid gap-4 sm:grid-cols-2">
          {PROBLEMS.map((item, i) => (
            <Reveal key={item.title} delay={i * 0.08}>
              <article className="group h-full rounded-3xl border border-white/8 bg-white/[0.025] p-7 transition duration-500 hover:border-nivo-gold/30 hover:bg-white/[0.045]">
                <p className="font-mono text-[10px] tracking-[0.24em] text-nivo-gold/80">
                  0{i + 1}
                </p>
                <h3 className="mt-5 font-serif text-2xl text-[#f4efe6]">{item.title}</h3>
        ...
        <Reveal className="mt-16">
          <p className="font-serif text-3xl text-nivo-gold-bright md:text-5xl">
            NIVO is built around transparency.
          </p>
        </Reveal>
      </Container>
    </section>
